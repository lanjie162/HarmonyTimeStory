...
```text
【模块 MN · 来源】请任选：① 主会话模拟 ② Task 子代理 ③ 沿用全流程（仅议程已公示时可用）。
...
> 决策点编号约定：`review-meeting` 用 `D-模块号-序号`、跨模块用 `G-序号`；`version-planning` 用 `P-序号`、变更用 `CR-序号`；`todo-drive` 用 `T-条目号`。具体规则见各 skill。
