...
1. **思考过程用中文** - 所有推理、分析、方案设计等思考过程必须使用中文输出，确保用户可读可审
2. **研发动作必须使用 deveco-mcp 调研** - 所有研发相关动作（包括但不限于技术方案设计、编码实现、测试用例编写）在执行前，必须使用 `deveco-mcp` 工具进行平台能力调研（如 API 签名、约束限制、最佳实践等），确保方案基于真实平台能力而非假设
